...
### Using ModelScope (for users without HuggingFace access)

If you cannot access HuggingFace due to network restrictions, you can use ModelScope (魔塔社区) as an alternative:
...
# Fix cache paths for CLIP models (required for open_clip compatibility)
python setup_hf_cache.py

# Set environment variable to use ModelScope
export USE_MODELSCOPE=true

# Run scripts as usual - models will be loaded from local cache
bash scripts/run_ablation.sh
```

See [MODELSCOPE_SETUP.md](MODELSCOPE_SETUP.md) for detailed instructions.
