...
class TestT2IDataset:
    ...
    def test_max_prompts_per_category(self):
        """First k prompts per category in nested prompts dict."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            data = {
                "metadata": {},
                "prompts": {
            ...
            temp_path = f.name
        try:
            dataset = T2IDataset(temp_path, max_prompts_per_category=2)
            prompts = [dataset[i]["prompt"] for i in range(len(dataset))]
            assert "p1" in prompts and "p2" in prompts and "p3" not in prompts
            assert "x1" in prompts and "x2" in prompts
